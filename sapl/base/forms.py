...
from sapl.rules import SGVP_GROUP_AUTOR, SGVP_GROUP_VOTANTE
...
class UserAdminForm(ModelForm):
                ...
                self.fields['user_permissions'].choices = [
                    ...
                    (p.id, p) for p in Permission.objects.filter(
                        content_type__app_label__in=list(
                            map(lambda x: x.split('.')[-1], settings.SGVP_APPS))
                    ).exclude(
                        user=self.instance
                    ).order_by('content_type__app_label',
                               'content_type__model',
    ...
    def save(self, commit=True):
        ...
        g_autor = Group.objects.get(name=SGVP_GROUP_AUTOR)
        g_votante = Group.objects.get(name=SGVP_GROUP_VOTANTE)
...
class AutorForm(ModelForm):
    ...
    operadores = forms.ModelMultipleChoiceField(
        ...
        label=_('Usuários do SGVP ligados ao autor acima selecionado'),
        required=False,
        help_text=_(
